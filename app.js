  ...
  try {
    ...
    if (data && data.records) {
      Object.keys(data.records).forEach(day => {
        ...
        if (dayEl && iconsEl) {
          let icons = '';
          if (record.sleep) icons += '😴';
          if (record.anxiety) icons += '😰';
          if (record.depression) icons += '😔';
          if (record.happiness) icons += '😊';
          if (record.exercise) icons += '🏃';
          if (record.drink) icons += '🥤';
          if (record.sitting) icons += '💺';
          if (record.ai) icons += '🤖';
...
function openDayDetail(day) {
  alert(`${currentYear}年${currentMonth + 1}月${day}日的详细记录\n\n功能开发中...`);
}
